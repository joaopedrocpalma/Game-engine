...
class aabbox3d
{
	public:
		...
		void setPosition(vector3df position, vector3df scale)	// Update collider position
		{
			MinEdge = vector3df(position.X - scale.X, position.Y - scale.Y, position.Z - scale.Z);
			MaxEdge = vector3df(position.X + scale.X, position.Y + scale.Y, position.Z + scale.Z);
		}

		void setGroundCollider(vector3df position, vector3df scale)	// Update player trigger collider
		{
			MinEdge.X = position.X - scale.X + 0.5f;
			MinEdge.Z = position.Z - scale.Z + 0.5f;
			MinEdge.Y = position.Y - scale.Y - 0.5f;

			MaxEdge.X = position.X + scale.X - 0.5f;
			MaxEdge.Y = position.Y - scale.Y - 0.001f;
			MaxEdge.Z = position.Z + scale.Z - 0.5f;
		}
